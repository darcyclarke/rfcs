...
Add a set of features to the npm cli that provide support to managing multiple packages from within a singular top-level package.

## Motivation

This feature has been requested by the community for a long time and the main motivation behind this popular clamor is the fact that in its current state the cli does not offer much in terms of managing multiple packages that may or may not be used together.
...
After sourcing feedback from the community, there are 3 major implementations/changes required in the cli in order to provide the feature set that would enable a better management of nested packages.
...
- Run npm subcommands in the context of each nested workspace
- Be able to bump versions and publish all nested workspaces
...
We can follow the lead of Yarn and support the `workspaces` `package.json` field which defines a list of paths, each of these paths may be a workspace itself but it also support globs.

`package.json` shorcut example:
...
A new config value should be introduced in order to allow for filtering out a subset of the packages in which to run these commands.
...
#### 4.1. Command that allow for bumping version of all package within a workspace

`npm versionall` or `npm workspaces version`: Walks the tree from the leafs to the root, bumping versions on each package and updating any previously matching dependency declaration in order to keep its declared semver range valid.

```
├── package.json { "name": "foo", "version": "0.1.0", "workspaces": ["dep-a", "dep-b", "dep-c"] }
├── dep-a
│   └── package.json { "version": "2.1.0", "dependencies": { "dep-b": "=2.1.0" } }
├── dep-b
│   └── package.json { "version": "2.1.0", "dependencies": { "dep-c": "^1.3.1" } }
└── dep-c
    └── package.json { "version": "1.3.1" }

$ npm versionall patch

├── package.json { "name": "foo", "version": "0.1.0", "workspaces": ["dep-a", "dep-b", "dep-c"] }
├── dep-a
│   └── package.json { "version": "2.1.1", "dependencies": { "dep-b": "=2.1.1" } }
├── dep-b
│   └── package.json { "version": "2.1.1", "dependencies": { "dep-c": "^1.3.0" } }
└── dep-c
    └── package.json { "version": "1.3.2" }
```

#### 4.2. Command that allow for publishing all workspaces packages

`npm publishall` or `npm workspaces publish`

## Examples

### 1. Symplistic example expanding on symlinking structure and `package-lock` file shape.
...
- If a package is a part of 2 workspaces simultaneously, it's a footgun (or is it?) -- ok to just skip any nested workspace packages that are already included in a root workspace, and vice versa.
- Proposed restriction: workspace packages must be contained by workspace folder (?)
